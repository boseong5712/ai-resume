...
수정 규칙:
- 원문의 사실관계, 회사명, 직무명, 경험, 수치, 핵심 메시지는 유지하세요.
- [개선 방향]과 [직접 요구사항]을 최우선으로 반영하세요.
- 직접 요구사항이 있으면 세부 개선 방향보다 직접 요구사항을 우선하세요.
- 선택된 개선 방향에 맞게 문체, 구조, 표현, 문단 흐름을 실제로 바꾸세요.
- AI가 쓴 듯한 반복적 표현은 줄이고, 자기소개서 전체 본문을 다시 작성하세요.
- 공백을 제외하고 1000자 이내로 정리하세요.
- 문단은 2~4개로 자연스럽게 나누세요.
- 설명이나 제목 없이 수정된 자기소개서 전체 본문만 출력하세요.`
    }
